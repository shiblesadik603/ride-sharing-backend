 ...
 * never the source of truth): `geo:drivers:online` is where the next
 * phase's push-dispatch will look up "who's near this pickup", written
 * here even though nothing reads it yet. `geo:rides:pending` is what
 * GET /rides/nearby actually queries today — a driver's own position
 * searched against pending ride requests. Postgres is never asked "what's
 * near X" directly; geo queries always go through Redis first.
 */
